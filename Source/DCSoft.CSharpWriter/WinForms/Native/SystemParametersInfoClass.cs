...
namespace DCSoft.WinForms.Native
{
    ...
    /// </summary>
    public sealed class SystemParametersInfoClass
    {

        [DllImport("user32.dll", CharSet = CharSet.Auto)]
        private static extern int SystemParametersInfo(SPIActions uAction, int uParam, IntPtr lpvParam, int fuWinIni);

        [DllImport("user32.dll", EntryPoint = "SystemParametersInfo", SetLastError = true)]
        private static extern bool SystemParametersInfoGet(SPIActions action, uint param, ref uint vparam, uint init);

        [DllImport("user32.dll", EntryPoint = "SystemParametersInfo", SetLastError = true)]
        private static extern bool SystemParametersInfoGetBool(SPIActions action, uint param, ref bool vparam, uint init);

        /// <summary>
        /// Windows操作系统是否是拖动时显示窗口内容
        /// </summary>
        public static bool DragFullWindows
        {
            ...
            {
                bool result = false;
                if (SystemParametersInfoGetBool(SPIActions.SPI_GETDRAGFULLWINDOWS, 0, ref result, 0))
                {
                    return result;
                }
                return false;
        ...
        /// </summary>
        /// <returns></returns>
        public static bool PowerOffActive
        {
            get
            {
                bool powerOffActive = false;
                if (SystemParametersInfoGetBool(SPIActions.SPI_GETPOWEROFFACTIVE, 0, ref powerOffActive, 0))
                {
        ...
        /// </summary>
        /// <returns></returns>
        public static int PowerOffTimeout
        {
            get
            {
                uint powerOffTimeout = 0;
                if (SystemParametersInfoGet(SPIActions.SPI_GETPOWEROFFTIMEOUT, 0, ref powerOffTimeout, 0))
                {
